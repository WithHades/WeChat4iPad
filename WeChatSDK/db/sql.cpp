...
void Sql3::Init(LPCSTR strDbNameUtf8)
{
	...
	if (nRes != SQLITE_OK)
	{
		printf("[Sql3::Init]Open database fail: %s", sqlite3_errmsg(m_pDB));
		return;
	}
	else
	{
		CStringA strDbNameUtf8 = CW2A(CUTF82W(m_strDbNameUtf8));
		printf("[Sql3::Init]Open database OK，database path:%s ", strDbNameUtf8);
	}
...
int Sql3::ExecSQL(LPCSTR strSQLUtf8, sqlite3_callback pCallBack /*= NULL*/, void *pUserArg /*= NULL*/)
{
	...
	printf("[Sql3::ExecSQL]正在执行Sql语句:%s", strSqlA);

	res = sqlite3_exec(m_pDB, strSQLUtf8, pCallBack, pUserArg, NULL);

	if (res != SQLITE_OK)
	{
		printf("[Sql3::ExecSQL][Error]sql:%s执行错误,错误信息:%s", strSqlA, sqlite3_errmsg(m_pDB));
	}
...
int Sql3::Begin()
{
	...
	if (ret != SQLITE_OK)
	{
		printf("[Sql3::Begin]sql开启事务失败,错误信息:%s", sqlite3_errmsg(m_pDB));
	}
...
int Sql3::Commit()
{
	...
	if (ret != SQLITE_OK)
	{
		printf("[Sql3::Begin]sql提交事务失败,错误信息:%s", sqlite3_errmsg(m_pDB));
	}
