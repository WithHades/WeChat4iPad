...
std::string IntoCompress(string in)
{
	string strCompressed;
	DWORD dwLenCompressed = compressBound(in.size());
...
std::string compress_rsa(string keye, string keyn, string in, DWORD &dwLenCompressed)
{
	...
	if (BN_hex2bn(&rsa->n, keyn.c_str()) == 0) { return strOut; };
	if (BN_hex2bn(&rsa->e, keye.c_str()) == 0) { return strOut; };
